...
begin
  case Status of
    ogValidCode    : S := 'Valid Code - Special Data: ' + IntToStr(OgSpecialCode1.GetValue);
...
begin
  S := '';
  {ask for code}
  if InputQuery('Special Code Entry', 'Enter the code', S) then begin
    Ini := TIniFile.Create('ONGUARD.INI');
    try
...
begin
  ...
  try
    IniFile.DeleteKey('Codes', 'SpecialCode');
  finally
    IniFile.Free;
  end;
  {$ELSE}
  WritePrivateProfileString('Codes', 'SpecialCode', nil, 'ONGUARD.INI');
  {$ENDIF}
