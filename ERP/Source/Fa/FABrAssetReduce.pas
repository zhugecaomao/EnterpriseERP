...
type
  TFABrAssetReduceForm = class(TWSVoucherBrowseForm)
...
var
  FABrAssetReduceForm: TFABrAssetReduceForm;

implementation

uses CommonDM,FAEdAssetReduce;

{$R *.dfm}

function TFABrAssetReduceForm.CreateEditForm: TWSEditForm;
begin
  Result := TFAEdAssetReduceForm.Create(Self);
end;

procedure TFABrAssetReduceForm.FormCreate(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Open;
end;

procedure TFABrAssetReduceForm.FormClose(Sender: TObject;
...
procedure TFABrAssetReduceForm.AddNewActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;

procedure TFABrAssetReduceForm.EditActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;

procedure TFABrAssetReduceForm.RefreshActionExecute(Sender: TObject);
begin
  inherited;
  UpdateDBGrid;
end;

procedure TFABrAssetReduceForm.UpdateDBGrid;
var  I: Integer;
begin
  with DBGrid do
  begin
...
procedure TFABrAssetReduceForm.FiltrateActionExecute(Sender: TObject);
begin
  inherited;
  UpdateDBGrid;
end;
