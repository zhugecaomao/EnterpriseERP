...
type
  TFNCashExchStatisticForm = class(TWSVoucherBrowseForm)
    ...
    AccountListQryDSDesigner8: TBCDField;
...
var
  FNCashExchStatisticForm: TFNCashExchStatisticForm;
...
function TFNCashExchStatisticForm.CreateEditForm: TWSEditForm;
begin
//  Result := TSLEdClearBillAForm.Create(Self);
end;


procedure TFNCashExchStatisticForm.UpdateDBGrid;
var I: Integer;
begin
  with DBGrid do
  begin
...
procedure TFNCashExchStatisticForm.DBGridTitleClick(Column: TColumn);
begin
  inherited;
  UpdateDBGrid;
end;

procedure TFNCashExchStatisticForm.DBGridDblClick(Sender: TObject);
begin
// inherited;
end;


procedure TFNCashExchStatisticForm.Button3Click(Sender: TObject);
begin
  inherited;
...
procedure TFNCashExchStatisticForm.FormShow(Sender: TObject);
var I :integer;
begin
  inherited;
  ADOQuery.Close;
  ...
  ADOQuery.SQL.Text :=' Select a.Date [日期], '
      ...
      +' Isnull(AmountD,0)*Isnull(ModeDC,1)*Isnull(ModeC,1) [收入原币金额]  ,      '
      ...
      +' Isnull(AmountD,0)*Isnull(ModeDC,1)*Isnull(ModeC,1)*(-1) [本币余额]  ,      '
      +' Isnull(AmountRate,0) [结存汇率]  ,                                    '
      +' Isnull(AmountOrigin,0)*Isnull(ModeDC,1)*Isnull(ModeC,1) [原币余额]  , '
  ...
  ADOQuery.SQL.Text :=' Update  #ExpenseList set '
      +' [收入本币金额]=Null,[收入原币金额]=Null,[收入汇率]=Null, '
      +' [本币余额]=Null,[原币余额]=Null,[结存汇率]=Null ';
  ADOQuery.ExecSQL;

  ADOQuery.Close;//兑换收入外币
