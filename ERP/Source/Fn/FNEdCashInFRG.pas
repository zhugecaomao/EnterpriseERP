...
type
  TFNEdCashInFRGForm = class(TBaseVoucherEditAForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  FNEdCashInFRGForm: TFNEdCashInFRGForm;
...
{$R *.dfm}
procedure TFNEdCashInFRGForm.New;
begin
  inherited;
  adsMaster.FieldByName('Date').AsDateTime :=date;
  adsMaster.FieldByName('Code').AsString:=GetMaxCode('Code','FNCashInMaster',number);
  adsMaster.FieldByName('CreateUserID').AsInteger :=Guarder.UserID;
  adsMaster.FieldByName('BillMode').AsString:='收款申请';
  adsMaster.FieldByName('ModeDC').AsInteger:=1;
  adsMaster.FieldByName('ModeC').AsInteger:=1;
  adsMaster.FieldByName('OriginTable').AsString:='FNCashInMaster';
  adsMaster.FieldByName('AmountC').AsFloat :=0;
end;

procedure TFNEdCashInFRGForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;


procedure TFNEdCashInFRGForm.FormClose(Sender: TObject;
...
procedure TFNEdCashInFRGForm.FormCreate(Sender: TObject);
begin
  inherited;
  VoucherTableName := 'FNCashIn';
  ClientName.Open;
  EmployeeName.Open;
  GoodName.Open;
  adsAccounts.Open;
  ...
  AdsProject.Open;
  adsBrief.Close;
  adsBrief.CommandText :=' select distinct Brief from FNCashInMaster';
  adsBrief.Open;
  adsBrief.first;
  ...
  if  adsBrief.IsEmpty then  BriefComboBox.Items.Add('收款');
end;

procedure TFNEdCashInFRGForm.ClientQLDBLookupEnter(Sender: TObject);
begin
  inherited;
  GEdit1.Enabled :=true;
  GEdit1.Text :='0';
...
procedure TFNEdCashInFRGForm.ClientQLDBLookupExit(Sender: TObject);
...
begin
  ...
  begin
    close;
    sql.Text := 'select sum(isnull(AmountD,0))-sum(isnull(AmountC,0)) '
        +' as Balance from (  '
...
procedure TFNEdCashInFRGForm.adsDetailExpenseIDChange(Sender: TField);
begin
  inherited;
  adsDetail.Edit;
...
procedure TFNEdCashInFRGForm.FormActivate(Sender: TObject);
...
procedure TFNEdCashInFRGForm.FormShow(Sender: TObject);
begin
  inherited;
//  ClientQLDBLookup.SetFocus;
end;

procedure TFNEdCashInFRGForm.LookUpAccountExit(Sender: TObject);
var StrAccountID :string;
begin
  inherited;
  StrAccountID := adsMaster.fieldbyname('AccountsID').AsString;
...
procedure TFNEdCashInFRGForm.DBEdit4Exit(Sender: TObject);
begin
  inherited;
