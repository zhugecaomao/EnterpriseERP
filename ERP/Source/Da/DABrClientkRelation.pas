...
type
  TDAClientkRelationBrowseForm = class(TWSStandardBrowseForm)
...
var
  DAClientkRelationBrowseForm: TDAClientkRelationBrowseForm;

implementation

uses DAEdmanufacturer, DABrClientk;

{$R *.dfm}
function TDAClientkRelationBrowseForm.CreateEditForm: TWSEditForm;
begin
  Result := TDAEdmanufacturerForm.Create(Application);
end;

procedure TDAClientkRelationBrowseForm.DBGridDblClick(Sender: TObject);
begin
//  inherited;
  DAClientkBrowseForm := TDAClientkBrowseForm.Create(self);
  with DAClientkBrowseForm do
  try
