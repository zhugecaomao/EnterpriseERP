...
type
  TSLEdSaleBForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
procedure TSLEdSaleBForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;


procedure TSLEdSaleBForm.FormClose(Sender: TObject;
...
procedure TSLEdSaleBForm.FormCreate(Sender: TObject);
...
procedure TSLEdSaleBForm.FormShow(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TSLEdSaleBForm.adsDetailGoodsIDChange(Sender: TField);
...
procedure TSLEdSaleBForm.adsDetailQuantityChange(Sender: TField);
var   ExChRate :real;
begin
...
procedure TSLEdSaleBForm.adsDetailPackUnitIDChange(Sender: TField);
var ExChRate :real;
begin
  PackUnit.Locate('ID', adsDetail.fieldbyname('PackUnitID').AsString, []);
  if PackUnit.fieldbyname('ExchangeRate').IsNull then ExChRate :=1
...
//  adsDetail.FieldByName('PackUnitID').AsInteger :=PackUnit.FieldByName('ID').AsInteger;
  adsDetail.fieldbyname('PriceBase').AsFloat :=ExChRate*
...
procedure TSLEdSaleBForm.adsDetailPriceBaseChange(Sender: TField);
begin
  inherited;
//  adsDetail.FieldByName('Amount').ReadOnly :=False;
  adsDetail.FieldByName('Amount').AsFloat :=
...
procedure TSLEdSaleBForm.SaveActionExecute(Sender: TObject);
...
procedure TSLEdSaleBForm.ClientQLDBLookupEnter(Sender: TObject);
begin
  inherited;
  GEdit1.Enabled :=true;
  GEdit1.Text :='0';
...
procedure TSLEdSaleBForm.ClientQLDBLookupExit(Sender: TObject);
var  adoTemp: TADOQuery;
begin
  inherited;
...
procedure TSLEdSaleBForm.FormActivate(Sender: TObject);
var  adoTemp: TADOQuery;
begin
  inherited;
  adoTemp := TADOQuery.Create(nil);
...
procedure TSLEdSaleBForm.FormDeactivate(Sender: TObject);
var  adoTemp: TADOQuery;
begin
  inherited;
  adoTemp := TADOQuery.Create(nil);
