...
type
  TSLEdGoodsOutBForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  SLEdGoodsOutBForm: TSLEdGoodsOutBForm;

implementation

uses CommonDM, WSUtils, WSSecurity;
{$R *.dfm}
procedure TSLEdGoodsOutBForm.New;
begin
  inherited;
...
//  adsMaster.FieldByName('BillMode').ReadOnly :=False;
  adsMaster.FieldByName('BillMode').AsString:='退货入库';
//  adsMaster.FieldByName('BillMode').ReadOnly :=True;
  adsMaster.FieldByName('ModeDC').AsInteger :=-1;
  adsMaster.FieldByName('ModeC').AsInteger :=1;
  adsMaster.FieldByName('OriginTable').AsString:='SLGoodsOutMaster';
  adsMaster.FieldByName('Deliver').AsString:='客户退回';
  adsMaster.FieldByName('Apportion').AsString:='--';
end;

procedure TSLEdGoodsOutBForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;


procedure TSLEdGoodsOutBForm.FormClose(Sender: TObject;
...
procedure TSLEdGoodsOutBForm.FormCreate(Sender: TObject);
...
procedure TSLEdGoodsOutBForm.FormShow(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TSLEdGoodsOutBForm.adsDetailGoodsIDChange(Sender: TField);
...
procedure TSLEdGoodsOutBForm.adsDetailQuantityChange(Sender: TField);
var   ExChRate :real;
begin
...
procedure TSLEdGoodsOutBForm.adsDetailPackUnitIDChange(Sender: TField);
var ExChRate :real;
begin
  PackUnit.Locate('ID', adsDetail.fieldbyname('PackUnitID').AsString, []);
  if PackUnit.fieldbyname('ExchangeRate').IsNull then ExChRate :=1
...
procedure TSLEdGoodsOutBForm.adsDetailPriceBaseChange(Sender: TField);
begin
  inherited;
//  adsDetail.FieldByName('Amount').ReadOnly :=False;
  adsDetail.FieldByName('Amount').AsFloat :=
...
procedure TSLEdGoodsOutBForm.FormActivate(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TSLEdGoodsOutBForm.LookupPackUintEnter(Sender: TObject);
