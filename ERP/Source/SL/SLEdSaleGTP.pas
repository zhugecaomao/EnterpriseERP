...
type
  TSLEdSaleGTPForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  SLEdSaleGTPForm: TSLEdSaleGTPForm;
...
{$R *.dfm}
procedure TSLEdSaleGTPForm.New;
begin
  inherited;
...
procedure TSLEdSaleGTPForm.Open(VoucherID: Integer);
var
...
procedure TSLEdSaleGTPForm.FormClose(Sender: TObject;
...
procedure TSLEdSaleGTPForm.FormCreate(Sender: TObject);
...
procedure TSLEdSaleGTPForm.adsDetailGoodsIDChange(Sender: TField);
...
procedure TSLEdSaleGTPForm.adsDetailPackUnitIDChange(Sender: TField);
var ExChRate :real;
begin
  PackUnit.Locate('ID', adsDetail.fieldbyname('PackUnitID').AsString, []);
  if PackUnit.fieldbyname('ExchangeRate').IsNull then ExChRate :=1
...
procedure TSLEdSaleGTPForm.NoCreditSaleClick(Sender: TObject);
begin
...
procedure TSLEdSaleGTPForm.DBEdit6Enter(Sender: TObject);
begin
  DBEdit6.ReadOnly :=not NoCreditSale.Checked;
  if DBEdit6.ReadOnly then
  begin
...
procedure TSLEdSaleGTPForm.InternalSave;
var adoTemp: TADOQuery;
...
procedure TSLEdSaleGTPForm.DiscountTempActClick(Sender: TObject);
var RateStr:string;
  RateStrF :Double;
begin
  inherited;
  ...
  begin
    adsDetail.Edit;
    adsDetail.FieldByName('PriceGoal').AsFloat :=
      adsDetail.FieldByName('PriceGoal').AsFloat*RateStrF/100;
...
procedure TSLEdSaleGTPForm.RepeatPrintActionExecute(Sender: TObject);
var RateStr:string;
  RateStrF :Double;
begin
  inherited;
