...
type
  TSLEdSaleBackForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  SLEdSaleBackForm: TSLEdSaleBackForm;
...
{$R *.dfm}
procedure TSLEdSaleBackForm.New;
begin
  inherited;
  ClientName.Open;
  EmployeeName.Open;
  ...
  adsSalesMaster.FieldByName('BillMode').Value:='销售退货开单';
...
procedure TSLEdSaleBackForm.Open(VoucherID: Integer);
begin
...
procedure TSLEdSaleBackForm.FormClose(Sender: TObject;
...
procedure TSLEdSaleBackForm.FormCreate(Sender: TObject);
begin
  inherited;
  VoucherTableName := 'SLSale';
end;

procedure TSLEdSaleBackForm.FormShow(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TSLEdSaleBackForm.ClientQLDBLookupEnter(Sender: TObject);
begin
  inherited;
  GEdit1.Enabled :=true;
  GEdit1.Text :='0';
...
procedure TSLEdSaleBackForm.ClientQLDBLookupExit(Sender: TObject);
...
procedure TSLEdSaleBackForm.DBComboBox3Change(Sender: TObject);
begin
  inherited;
...
procedure TSLEdSaleBackForm.GoodsIDChange;
begin
  TempAds.Close;   //取标准单位
...
procedure TSLEdSaleBackForm.QuantityChange;
begin
...
procedure TSLEdSaleBackForm.adsSalesDetailPackUnitIDChange(Sender: TField);
begin
  inherited;
  adsSalesDetail.FieldByName('GoalQuantity').ReadOnly :=False;
  adsSalesDetail.FieldByName('GoalQuantity').AsFloat :=
...
procedure TSLEdSaleBackForm.adsSalesDetailQuantityChange(Sender: TField);
begin
  inherited;
  QuantityChange;
end;

procedure TSLEdSaleBackForm.adsSalesDetailGoodsIDChange(Sender: TField);
begin
  inherited;
  GoodsIDChange;
end;

procedure TSLEdSaleBackForm.SaveActionExecute(Sender: TObject);
var adoTemp: TADOQuery;
    code:string;
begin
  inherited;
...
procedure TSLEdSaleBackForm.ImportActionExecute(Sender: TObject);
begin
  inherited;
...
procedure TSLEdSaleBackForm.LastPriceExecute(Sender: TObject);
begin
  inherited;
...
procedure TSLEdSaleBackForm.DiscountModeExecute(Sender: TObject);
begin
  inherited;
