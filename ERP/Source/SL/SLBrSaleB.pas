...
type
  TSLBrSaleBForm = class(TWSVoucherBrowseForm)
...
var
  SLBrSaleBForm: TSLBrSaleBForm;

implementation

uses CommonDM,SLEdSaleB,WSSecurity,SLEdSaleBGTP;

{$R *.dfm}

function TSLBrSaleBForm.CreateEditForm: TWSEditForm;
begin
  if Guarder.PackToGoalUnit='ÊÇ' then
    Result := TSLEdSaleBForm.Create(Self)
  else Result := TSLEdSaleBGTPForm.Create(Self);
end;

procedure TSLBrSaleBForm.FormCreate(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Open;
end;

procedure TSLBrSaleBForm.FormClose(Sender: TObject;
...
procedure TSLBrSaleBForm.AddNewActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;

procedure TSLBrSaleBForm.EditActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;
