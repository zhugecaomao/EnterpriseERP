...
type
  TSLEdSalePriceForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  SLEdSalePriceForm: TSLEdSalePriceForm;

implementation

uses CommonDM, WSUtils, WSSecurity;
{$R *.dfm}
procedure TSLEdSalePriceForm.New;
begin
  inherited;
  adsMaster.FieldByName('Date').AsDateTime :=date;
  adsMaster.FieldByName('Code').AsString:=GetMaxCode('Code','SLSalePriceMaster ',number);
  adsMaster.FieldByName('CreateUserID').AsInteger :=Guarder.UserID;
end;

procedure TSLEdSalePriceForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;


procedure TSLEdSalePriceForm.FormClose(Sender: TObject;
...
procedure TSLEdSalePriceForm.FormCreate(Sender: TObject);
begin
  inherited;
  VoucherTableName := 'SLSalePrice';
  ClientName.Open;
  GoodName.Open;
  PackUnit.Open;
  TempAds.Close;
  TempAds.CommandText :=' select Distinct ContractClass from SLSalePriceMaster ';
  TempAds.Open;
...
procedure TSLEdSalePriceForm.adsDetailGoodsIDChange(Sender: TField);
var SUnitID :integer;
begin
  inherited;
...
procedure TSLEdSalePriceForm.LookupPackUintEnter(Sender: TObject);
...
procedure TSLEdSalePriceForm.FormActivate(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TSLEdSalePriceForm.LookupPackUintExit(Sender: TObject);
begin
//  LookupPackUint.ListSource := dsPackUnit;
end;

procedure TSLEdSalePriceForm.GoodsIDSelectBttClick(Sender: TObject);
var
