...
procedure TSLEdSaleForm.adsDetailQuantityChange(Sender: TField);
var   ExChRate :real;
begin
...
procedure TSLEdSaleForm.ClientQLDBLookupEnter(Sender: TObject);
begin
  inherited;
  GEdit1.Enabled :=true;
  GEdit1.Text :='0';
...
procedure TSLEdSaleForm.ClientQLDBLookupExit(Sender: TObject);
...
procedure TSLEdSaleForm.FormActivate(Sender: TObject);
var  adoTemp: TADOQuery;
begin
  inherited;
  adoTemp := TADOQuery.Create(nil);
...
procedure TSLEdSaleForm.FormDeactivate(Sender: TObject);
var  adoTemp: TADOQuery;
begin
  inherited;
  adoTemp := TADOQuery.Create(nil);
...
function TSLEdSaleForm.CreateReport: TQuickRep;
begin
  Result := TSLSaleVoucherReport.Create(Self);
  TBaseVoucherReport(Result).SetMasterDataSet(MasterDataSet);
end;

procedure TSLEdSaleForm.FillInPrintActionExecute(Sender: TObject);
