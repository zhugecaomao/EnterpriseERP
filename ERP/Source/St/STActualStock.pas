...
type
  TSTActualStockForm = class(TWSVoucherBrowseForm)
...
var
  STActualStockForm: TSTActualStockForm;
...
function TSTActualStockForm.CreateEditForm: TWSEditForm;
begin
//  Result := TSLEdClearBillAForm.Create(Self);
end;


procedure TSTActualStockForm.UpdateDBGrid;
var I: Integer;
begin
  with DBGrid do
  begin
...
procedure TSTActualStockForm.DBGridTitleClick(Column: TColumn);
begin
  inherited;
  UpdateDBGrid;
end;

procedure TSTActualStockForm.FormActivate(Sender: TObject);
begin
  inherited;
  ADOQuery.Close;
  ADOQuery.SQL.Text :=' IF EXISTS(  SELECT * FROM tempdb..sysobjects '
        +' WHERE ID = OBJECT_ID('+Quotedstr('tempdb..#STActualStockTtl')
        +' )) DROP TABLE #STActualStockTtl ' ;
  ADOQuery.ExecSQL;
  ADOQuery.Close;
  ADOQuery.SQL.Text :=' CREATE TABLE #STActualStockTtl  ('
  ...
  ADOQuery.Close;
  ADOQuery.SQL.Text :=' IF EXISTS(  SELECT * FROM tempdb..sysobjects '
        +' WHERE ID = OBJECT_ID('+Quotedstr('tempdb..#STActualStockDM')
        +' )) DROP TABLE #STActualStockDM ' ;
  ADOQuery.ExecSQL;
  ADOQuery.Close;
  ADOQuery.SQL.Text :=' CREATE TABLE #STActualStockDM  ('
  ...
  ADOQuery.Close;
  ADOQuery.SQL.Text :=' TRUNCATE TABLE #STActualStockDM';
  ADOQuery.ExecSQL;
//入库业务包含:采购入库,生产入库,调拨出库,盘点赢余
// PCGoodsInMaster,YDGoodsInMaster,STGoodsOutInMaster,STGoodsCountOffMaster
  ADOQuery.Close;  // 采购入库记录
  ADOQuery.SQL.Text :=' INSERT INTO #STActualStockDM ('
    ...
    +' from PCGoodsinDetail AS a                                       '
    +' LEFT OUTER JOIN PCGoodsinMaster  as b  on a.MasterID=b.id         '
