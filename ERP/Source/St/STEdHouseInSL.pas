...
begin
  inherited;
  adsSTHouseOutMaster.FieldByName('HouseOut').AsString :='ÍË»õÈë¿â';
...
begin
  ...
  begin
    close;
    sql.Text := 'select * from DABillType where  RecordState<>' + QuotedStr('É¾³ý') +
       ' and BillFlagST=' + QuotedStr('1')+ ' AND Name LIKE '+QuotedStr('%ÍË»õ%');
    open;
