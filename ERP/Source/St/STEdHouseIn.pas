...
type
  TSTHouseInEditForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    Label5: TLabel;
...
var
  STHouseInEditForm: TSTHouseInEditForm;
...
procedure TSTHouseInEditForm.New;
begin
  inherited;
  adsSTHouseOutMaster.FieldByName('HouseOut').AsString :='²É¹ºÈë¿â';
  adsSTHouseOutMaster.FieldByName('Code').AsString:=GetMaxCode('Code','STHouseInMaster',Ascii);
  adsSTHouseOutMaster.FieldByName('Date').AsString:=FormatDateTime('yyyy-mm-dd',date());
end;

procedure TSTHouseInEditForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;

procedure TSTHouseInEditForm.FormCreate(Sender: TObject);
var
  adoTemp: TADOQuery;
begin
  inherited;
...
procedure TSTHouseInEditForm.ImportActionExecute(Sender: TObject);
begin
  inherited;
...
procedure TSTHouseInEditForm.adsSTHouseOutDetailGoodsIDChange(
  Sender: TField);
begin
  inherited;
...
procedure TSTHouseInEditForm.adsSTHouseOutDetailNewRecord(
  DataSet: TDataSet);
begin
  inherited;
  try
...
procedure TSTHouseInEditForm.ADOTable3FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  inherited;
...
procedure TSTHouseInEditForm.ADOTable4FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  inherited;
accept := not (dataset.FieldByName('RecordState').AsString = 'É¾³ý');
