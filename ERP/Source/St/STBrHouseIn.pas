...
type
  TSTHouseInBrowseForm = class(TWSVoucherBrowseForm)
...
var
  STHouseInBrowseForm: TSTHouseInBrowseForm;

implementation

uses STEdHouseIn, CommonDM;

{$R *.dfm}

function TSTHouseInBrowseForm.CreateEditForm: TWSEditForm;
begin
  Result := TSTHouseInEditForm.Create(Application);
end;

procedure TSTHouseInBrowseForm.AddNewActionExecute(Sender: TObject);
begin
  inherited;
  ADODataSet1.Requery(); 
end;

procedure TSTHouseInBrowseForm.DeleteActionExecute(Sender: TObject);
begin
  inherited;
  ADODataSet1.Requery();
end;

procedure TSTHouseInBrowseForm.EditActionExecute(Sender: TObject);
begin
  inherited;
  ADODataSet1.Requery();
end;
