...
type
  TSTHouseOutPCEditForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    Label5: TLabel;
...
var
  STHouseOutPCEditForm: TSTHouseOutPCEditForm;
...
procedure TSTHouseOutPCEditForm.New;
begin
  inherited;
  adsSTHouseOutMaster.FieldByName('HouseOut').AsString := 'ÍË»õ³ö¿â';
  adsSTHouseOutMaster.FieldByName('Code').AsString := GetMaxCode('Code', 'STHouseOutMaster', number);
  adsSTHouseOutMaster.FieldByName('Date').AsString := FormatDateTime('yyyy-mm-dd', date());
end;

procedure TSTHouseOutPCEditForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;

procedure TSTHouseOutPCEditForm.FormCreate(Sender: TObject);
var
  adoTemp: TADOQuery;
begin
  inherited;
...
procedure TSTHouseOutPCEditForm.ImportActionExecute(Sender: TObject);
begin
  inherited;
...
procedure TSTHouseOutPCEditForm.adsSTHouseOutDetailGoodsIDChange(
  Sender: TField);
begin
  inherited;
...
procedure TSTHouseOutPCEditForm.adsSTHouseOutDetailNewRecord(
  DataSet: TDataSet);
begin
  inherited;
  try
...
procedure TSTHouseOutPCEditForm.ADOTable3FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  inherited;
...
procedure TSTHouseOutPCEditForm.ADOTable4FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  inherited;
accept := not (dataset.FieldByName('RecordState').AsString = 'É¾³ý');
