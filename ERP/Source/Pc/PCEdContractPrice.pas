...
type
  TPCEdContractPriceForm = class(TBaseVoucherEditForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  PCEdContractPriceForm: TPCEdContractPriceForm;

implementation

uses CommonDM, WSUtils, WSSecurity;
{$R *.dfm}
procedure TPCEdContractPriceForm.New;
begin
  inherited;
  adsMaster.FieldByName('Date').AsDateTime :=date;
  adsMaster.FieldByName('Code').AsString:=GetMaxCode('Code','PCContractPriceMaster ',number);
  adsMaster.FieldByName('CreateUserID').AsInteger :=Guarder.UserID;
end;

procedure TPCEdContractPriceForm.Open(VoucherID: Integer);
begin
  inherited Open(VoucherID);
end;


procedure TPCEdContractPriceForm.FormClose(Sender: TObject;
...
procedure TPCEdContractPriceForm.FormCreate(Sender: TObject);
begin
  inherited;
  VoucherTableName := 'PCContractPrice';
  ClientName.Open;
  GoodName.Open;
  PackUnit.Open;
  TempAds.Close;
  TempAds.CommandText :=' select Distinct ContractClass from PCContractPriceMaster ';
  TempAds.Open;
...
procedure TPCEdContractPriceForm.adsDetailGoodsIDChange(Sender: TField);
var SUnitID :integer;
begin
  inherited;
...
procedure TPCEdContractPriceForm.LookupPackUintEnter(Sender: TObject);
...
procedure TPCEdContractPriceForm.FormActivate(Sender: TObject);
begin
  inherited;
  ClientQLDBLookup.SetFocus;
end;

procedure TPCEdContractPriceForm.LookupPackUintExit(Sender: TObject);
begin
//  LookupPackUint.ListSource := dsPackUnit;
end;

procedure TPCEdContractPriceForm.GoodsIDSelectBttClick(Sender: TObject);
var
...
//    GoodsIDSelectBtt.Enabled:=False;
    ...
    ' a.PricePurchase FROM DAGoods a '+
...
//      adsDetail.FieldByName('PackUnitID').AsInteger := GoodsSelectADS.fieldbyname('GoalUnitID').AsInteger;
      adsDetail.FieldByName('PriceClear').AsFloat     := GoodsSelectADS.fieldbyname('PricePurchase').AsFloat;
//      adsDetail.Post;
      GoodsSelectADS.Next;
      adsDetail.Next;
    end;
...
procedure TPCEdContractPriceForm.HoldOnPriceActionExecute(Sender: TObject);
var RateStr:string;
  RateStrF,I :Integer;
  BaseDiscount:real;
begin
  inherited;
  RateStr :='1';
  if InputQuery('商品连续单价', '请输入连续数量:', RateStr) then
    while not TryStrToInt(RateStr,RateStrF) do
     if not InputQuery('商品连续单价', '请输入连续数量:', RateStr) then exit;
