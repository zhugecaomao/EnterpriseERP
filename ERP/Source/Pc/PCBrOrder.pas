...
type
  TPCBrOrderForm = class(TWSVoucherBrowseForm)
...
var
  PCBrOrderForm: TPCBrOrderForm;

implementation

uses CommonDM, PCEdOrder,PCEdOrderGTP,WSSecurity;

{$R *.dfm}

function TPCBrOrderForm.CreateEditForm: TWSEditForm;
begin
  if Guarder.PackToGoalUnit='ÊÇ' then
    Result := TPCEdOrderForm.Create(Self)
  else Result := TPCEdOrderGTPForm.Create(Self);
end;

procedure TPCBrOrderForm.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TPCBrOrderForm.FormClose(Sender: TObject;
...
procedure TPCBrOrderForm.AddNewActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;

procedure TPCBrOrderForm.EditActionExecute(Sender: TObject);
begin
  inherited;
  adsSalesMaster.Requery();
end;
